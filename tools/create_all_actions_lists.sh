...
do
    ...
    echo "Creating Last 20000 ${group} list"
    grep $group /tmp/all | head -5000 | gsutil cp - gs://abadia-data/last_20000${group}_list.txt

    echo "Creating Last 5000 ${group} list"
    grep $group /tmp/all | head -1000 | gsutil cp - gs://abadia-data/last_5000_${group}_list.txt
done
