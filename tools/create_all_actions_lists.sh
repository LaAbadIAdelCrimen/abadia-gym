...
for group in "abadia_actions" "abadia_games" "abadia_values" ".checkpoint"
do
    echo "Creating all $group list gs://abadia-data/all_${group}_list.txt"
    grep "$group" /tmp/all | gsutil cp -I gs://abadia-data/all_${group}_list.txt

    echo "Creating Last 5000 ${group} list"
    grep "$group" /tmp/all | head -5000 | gsutil cp -I gs://abadia-data/last_5000_${group}_list.txt

    echo "Creating Last 1000 ${group} list"
    grep "$group" /tmp/all | head -1000 | gsutil cp -I gs://abadia-data/last_1000_${group}_list.txt
done
